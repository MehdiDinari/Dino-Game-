...
class Dino(pygame.sprite.Sprite):
    ...
    def jump(self):
        if not self.ducking and self.rect.centery == 360:
            self.jumping = True
            self.velocity = -22
            jump_sfx.play()
    ...
    def apply_gravity(self):
        if self.jumping:
            self.rect.centery += self.velocity  # Applique la vitesse (monte ou descend)
            self.velocity += self.gravity  # Augmente la vitesse (gravité)

            if self.rect.centery >= 360:  # Si Dino touche le sol
                self.rect.centery = 360
                self.jumping = False
                self.velocity = 0  # Réinitialise la vitesse
